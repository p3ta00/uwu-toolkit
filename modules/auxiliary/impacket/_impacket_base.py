...
IMPACKET_TOOLS = {
    ...
    "owneredit": {
        ...
        "domain_required": True,
        "extra_options": [
            {"name": "TARGET_DN", "desc": "Target distinguished name", "required": True},
            {"name": "NEW_OWNER", "desc": "New owner SID or name", "required": True},
...
class ImpacketModule(ModuleBase):
    ...
    def _build_target_string(self) -> str:
        """Build [domain/]user[:pass]@target connection string."""
    ...
    def run(self) -> bool:
        ...
        if tool_path:
            ...
            if interactive:
                ret = self.run_in_exegol_interactive(cmd_str)
                self.print_line()
                return ret == 0
            else:
