...
class ADAttackEnum(ModuleBase):
    ...
    def _enum_acl(self, dc_ip: str, domain: str, user: str,
        ...
        cmd = f"bloodyAD -u {shlex.quote(user)} -p {shlex.quote(password)} -d {shlex.quote(domain)} --host {dc_ip} get writable --detail"
        ...
        # Resolve sAMAccountName for each writable object via LDAP
        # CN in the DN often differs from sAMAccountName (e.g. CN=StellarOps-Control vs ops.controller)
        for obj in self.findings["writable_objects"]:
            dn = obj["dn"]
            sam_cmd = (
                f"ldapsearch -x -H ldap://{dc_ip} -D {shlex.quote(f'{user}@{domain}')} -w {shlex.quote(password)} "
                f"-b {shlex.quote(dn)} -s base sAMAccountName"
            )
            ret, stdout, stderr = self.run_in_exegol(sam_cmd, timeout=30)
            sam_output = stdout + stderr
            sam_match = re.search(r'sAMAccountName:\s*(\S+)', sam_output)
            if sam_match:
                obj["sam_account_name"] = sam_match.group(1)
            else:
                # Fallback: try to derive from CN
                obj["sam_account_name"] = obj["cn"]
        ...
        if self.findings["writable_objects"]:
            ...
            for obj in self.findings["writable_objects"][:10]:  # Show first 10
                ...
                sam = obj.get("sam_account_name") or cn
                display = f"{sam}" if sam != cn else cn
                if sam != cn:
                    display = f"{sam} (CN={cn})"
    ...
    def _enum_adcs(self, dc_ip: str, domain: str, user: str,
        ...
        certipy_path = _find_certipy()
        cmd = f"{certipy_path} find -u {shlex.quote(f'{user}@{domain}')} -p {shlex.quote(password)} -dc-ip {dc_ip} -stdout -vulnerable"
    ...
    def _enum_delegation(self, dc_ip: str, domain: str, user: str,
        ...
        cmd = f"ldapsearch -x -H ldap://{dc_ip} -D {shlex.quote(f'{user}@{domain}')} -w {shlex.quote(password)} -b {shlex.quote(base_dn)} '(userAccountControl:1.2.840.113556.1.4.803:=524288)' cn dNSHostName"
        ...
        cmd = f"ldapsearch -x -H ldap://{dc_ip} -D {shlex.quote(f'{user}@{domain}')} -w {shlex.quote(password)} -b {shlex.quote(base_dn)} '(msDS-AllowedToDelegateTo=*)' sAMAccountName msDS-AllowedToDelegateTo"
    ...
    def _analyze_attack_paths(self) -> None:
        ...
        for obj in self.findings["writable_objects"]:
            ...
            if "unicodePwd" in perms_str or "OWNER" in perms_str:
                path = {
                    "name": f"ACL Abuse: Reset {sam}'s password",
                    "steps": [
                        f"1. Use bloody_setpass to reset {sam}'s password",
                        f"2. Authenticate as {sam}",
                        f"3. Enumerate new attack surface"
                    ],
                    "risk": "CRITICAL",
                    "module": "auxiliary/ad/bloody_setpass",
                    "command": f"set TARGET_USER {sam}"
                }
                paths.append(path)

            if "servicePrincipalName" in perms_str:
                path = {
                    "name": f"Targeted Kerberoast: Add SPN to {sam}",
                    "steps": [
                        f"1. Use bloody_spn to add SPN to {sam}",
...
"""
    ...
    def check(self) -> bool:
        ...
        if certipy == "certipy":
            ...
            if ret != 0:
                self.print_warning("Certipy not found - AD CS enumeration will fail")
