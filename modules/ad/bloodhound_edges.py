...
class BloodHoundEdges(ModuleBase):
    ...
    def run(self) -> bool:
        ...
        # Clear suggestions from previous run
        self._suggested_modules = []
    ...
    def _print_edge_card(self, name: str, data: dict):
        ...
        if modules:
            ...
            for mod_path, context in modules:
                # Global index across all cards in this run
                self._suggested_modules.append((mod_path, context))
                idx_num = len(self._suggested_modules)
                styled_path = Style.module_path(mod_path)
                idx = f"{Colors.DIM}[{idx_num}]{R}"
