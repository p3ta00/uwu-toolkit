...
# Certipy lives in venvs, not on standard PATH
CERTIPY_SEARCH_PATHS = [
...
def _find_certipy() -> str:
    """Find certipy binary, checking venv paths"""
    import os
    path = find_tool("certipy")
    if path:
        return path
    for p in CERTIPY_SEARCH_PATHS:
        if os.path.isfile(p) and os.access(p, os.X_OK):
            return p
    return "certipy"  # fallback to PATH
...
class CertipyFind(ModuleBase):
    ...
    def run(self) -> bool:
        ...
        # Build command - find certipy dynamically
        cmd_parts = [
            _find_certipy(),
            "find",
            "-u", f"{user}@{domain}",
